    ...
    public class KeyGrabber
    {
        public class KeyFilter : IMessageFilter
        {
            public bool PreFilterMessage(ref Message m)
            {
                ...
                const int WCHAR_EVENT = 0x102;

                if (m.Msg == WM_KEYDOWN)
                {
                    /* The TranslateMessage function requires a pointer to be passed to it.
                     * Since C# doesn't doesn't typically use pointers, we have to make use of the Marshal
                ...
                else if (m.Msg == WCHAR_EVENT)
                {
                     ...
                     * notified that there is a char ready to be processed.
                     */
